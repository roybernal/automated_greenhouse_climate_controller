...
// ---------------------------
// Definición de Pines CORREGIDOS
// ---------------------------
#define DHTPIN D2
#define DHTTYPE DHT11

const int growLightPin = D1;    // Luces de crecimiento
const int heaterPin = D5;       // Calefactor
...
// ---------------------------
// Enviar Datos a Firebase CORREGIDO
// ---------------------------
void sendDataToFirebase(float temp, float hum, int lightValue, int soilMoisture) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (http.begin(*client, url_actuators)) {
      ...
      String heaterStatus = (digitalRead(heaterPin) == HIGH) ? "true" : "false";
      String lightsStatus = (digitalRead(growLightPin) == HIGH) ? "true" : "false";
      String irrigationStatus = (digitalRead(irrigationRelayPin) == LOW) ? "true" : "false";

      String jsonActuators = "{\"fan\":" + fanStatus + 
                             ",\"heater\":" + heaterStatus + 
                             ",\"led_light\":" + lightsStatus +
...
void setup() {
  ...
  digitalWrite(heaterPin, LOW);           // OFF (Active HIGH)  
  digitalWrite(growLightPin, LOW);        // OFF (Active HIGH)
...
// ---------------------------
// Verificar Controles de Firebase CORREGIDO
// ---------------------------
void checkFirebaseControls() {
  ...
  if (http.begin(*client, url_controls)) {
    ...
    if (httpCode == 200) {
      ...
      // Control para Ventilador (fan)
      if (doc.containsKey("fan")) {
        bool fanState = doc["fan"];
        digitalWrite(fanRelayPin, fanState ? LOW : HIGH);
        Serial.println(fanState ? "Ventilador: ON" : "Ventilador: OFF");
      }

      // Control para Calefactor (heater)
      if (doc.containsKey("heater")) {
        bool heaterState = doc["heater"];
        digitalWrite(heaterPin, heaterState ? HIGH : LOW);
        Serial.println(heaterState ? "Calefactor: ON" : "Calefactor: OFF");
      }

      // Control para Luces de Crecimiento (led_light)
      if (doc.containsKey("led_light")) {
        bool lightsState = doc["led_light"];
        digitalWrite(growLightPin, lightsState ? HIGH : LOW);
        Serial.println(lightsState ? "Luces: ON" : "Luces: OFF");
      }

      // Control para Riego (irrigation)
      if (doc.containsKey("irrigation")) {
        bool irrigationState = doc["irrigation"];
        digitalWrite(irrigationRelayPin, irrigationState ? LOW : HIGH);
        Serial.println(irrigationState ? "Riego: ON" : "Riego: OFF");
      }
...
void loop() {
  ...
  // Depuración de estados de pines
  Serial.println("=== ESTADO DE ACTUADORES ===");
  Serial.print("Fan (D6): "); Serial.println(digitalRead(fanRelayPin) == LOW ? "ON" : "OFF");
  Serial.print("Heater (D5): "); Serial.println(digitalRead(heaterPin) == HIGH ? "ON" : "OFF");
  Serial.print("Lights (D1): "); Serial.println(digitalRead(growLightPin) == HIGH ? "ON" : "OFF");
  Serial.print("Irrigation (D7): "); Serial.println(digitalRead(irrigationRelayPin) == LOW ? "ON" : "OFF");
  Serial.println("=============================");
